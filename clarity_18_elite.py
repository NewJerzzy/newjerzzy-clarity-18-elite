...
def run_dashboard():
    ...
    tab1, tab2, tab3, tab4 = st.tabs([
        "🎮 GAME MARKETS", "📋 PASTE & SCAN", "📊 SCANNERS & ACCURACY", "🔧 SELF EVALUATION"
    ])
    ...
    **📅 Optimal Scanning Windows (for best lines & player props):**
    ...
    with tab2:
        ...
        pasted_text = st.text_area("Paste here", height=300, key="paste_scan_text",
        ...
        with col1:
            if st.button("🔍 ANALYZE & IMPORT", type="primary", use_container_width=True):
                if not pasted_text.strip():
                    st.warning("Please paste something to analyze.")
                else:
                    approved_props = []
                    imported_bets = []
                    rejected_items = []
