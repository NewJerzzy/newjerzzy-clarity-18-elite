...
# =============================================================================
# REGRESSION TO THE MEAN, FATIGUE, INJURY FETCH, LINE MOVEMENT
# =============================================================================
...
def analyze_prop(
    ...
    mu = (mu_raw * b2b_mult * travel_mult * alt_mult * weather_mult *
          motivation_mult * series_mult * news_mult * fatigue_mult)
    ...
    spread_val = None  # would need game spread; skip for now, but keep structure
    if sport.upper() == "NBA" and enable_six_condition:
        # Simplified: condition 5 (defensive red‑flag) and condition 3 (spread) omitted
        # Condition 1: usage spike (usage_trend_up as proxy)
        # Condition 2: pace advantage (assume true)
        # Condition 4: last‑10 logs already present
    ...
    if sport.upper() == "NBA" and spread_val is not None and abs(spread_val) >= 10:
        ...
        if market.upper() == "TOTAL" and pick.upper() == "OVER":
            mu *= 0.95  # downgrade but not auto‑pass
    # =======================================================================
...
SPORTS = {
    ...
    "SOCCER": "soccer_epl", "TENNIS": "tennis_atp"
}
...
# =============================================================================
# STREAMLIT UI – 8 TABS (with all new features)
# =============================================================================
_BADGE_CSS = {
        ...
        if players:
            ...
            with st.spinner(f"Pricing {len(players)} players..."):
                for _, row in player_cols.iterrows():
                    ...
                    if len(stats) >= 4:
                        sigma = max(compute_wsem(stats) * l42_buffer(stats), 0.75)
                    else:
                        sigma = max(1.5, mu * 0.25)
                    ...
                    results.append({
                        "Player": pname, "Market": mtype, "Line": sb_line,
                        "Fair Line": round(mu, 2), "P(over)": round(p_over, 3),
                        "Edge": round(edge, 3), "Kelly": round(kelly_frac, 3),
                        "Tier": classify_tier(edge), "Confidence": confidence_score(len(stats))
                    })
            if results:
